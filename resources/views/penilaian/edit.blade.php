...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">Edit Penilaian: {{ $alternative->nama }}</h5>
    </div>
    <div class="card-body">
        ...
        <form action="{{ route('penilaian.update', $alternative) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                @foreach($criterias as $criteria)
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="scores[{{ $criteria->id }}]" 
                                class="form-select @error('scores.'.$criteria->id) is-invalid @enderror"
                                required>
                            <option value="">Pilih Nilai</option>
                            @for($i = 1; $i <= 5; $i++)
                                <option value="{{ $i }}" 
                                    {{ optional($alternative->scores->where('criteria_id', $criteria->id)->first())->nilai == $i ? 'selected' : '' }}>
                                    {{ $i }}
                                </option>
                            @endfor
                        </select>
